...
export const useAppointmentsStore = defineStore('appointments', () =>{
    ...
    watch(date,async() =>{
        ...
        if(appointmentId.value){
            ...
            const currentAppointment = data.filter( appointment=> appointment._id === appointmentId.value)[0].time
    ...
    async function cancelAppointment(id){
        if(confirm('¿Desas eliminar esta cita?')){
            try {
                ...
                user.userAppointments = user.userAppointments.filter(appointement => appointement._id !== id)
            } catch (error) {
                toast.open({
